...
$CFG_Version     = "2.1"
$CFG_CompanyName = "IT Department"          # for documentation only

# -- Driver type ----------------------------------------------------
# Set to "EXE" if your driver ships as a setup executable.
# Set to "INF" if your driver ships as an .inf file.
$CFG_DriverType = "EXE"

# -- Driver path ----------------------------------------------------
# Path to the driver file, relative to this script's folder.
#
# EXE example:  "install\pcl6\install.exe"
# INF example:  "install\driver\oemsetup.inf"
$CFG_DriverRelPath = "install\pcl6\install.exe"

# -- EXE: silent install arguments ----------------------------------
# Arguments passed to the installer when CFG_DriverType = "EXE".
# Ignored when CFG_DriverType = "INF".
#
# HP UPD example:   '/s /v"/qn REBOOT=ReallySuppress"'
# Generic MSI:      '/quiet /norestart'
# NSIS installer:   '/S'
$CFG_DriverSilentArgs = '/s /v"/qn REBOOT=ReallySuppress"'

# -- Driver detection pattern ---------------------------------------
# Wildcard pattern used to find the driver in Get-PrinterDriver.
# The script checks this BEFORE installing and AFTER installing.
#
# HP UPD PCL 6:     "*HP*Universal*PCL*6*"
# HP UPD PS:        "*HP*Universal*PostScript*"
# Canon UFRII:      "*Canon*UFRII*"
# Kyocera KX:       "*Kyocera*KX*"
# Generic:          "*YourDriverName*"
$CFG_DriverPattern = "*HP*Universal*PCL*6*"

# -- Printer list file ----------------------------------------------
# Name of the JSON file that stores the printer list.
# Must be in the same folder as this script.
$CFG_JsonFile = "printers.json"

# -- Port name prefix -----------------------------------------------
# TCP/IP ports are named <PREFIX><IP>,  e.g. "PRN-192.168.1.101"
# Change if the default prefix conflicts with existing ports.
$CFG_PortPrefix = "PRN-"

# -- Colors  (ANSI 24-bit RGB  "R;G;B") ----------------------------
...
$ESC    = [char]27
$CR     = "$ESC[0m"
$HEADER = "$ESC[38;2;$($CFG_Color_Header)m"
$ACCENT = "$ESC[38;2;$($CFG_Color_Accent)m"
$COK    = "$ESC[38;2;$($CFG_Color_OK)m"
$CWARN  = "$ESC[38;2;$($CFG_Color_Warn)m"
$CERR   = "$ESC[38;2;$($CFG_Color_Err)m"
$CDIM   = "$ESC[38;2;$($CFG_Color_Dim)m"
$CWHITE = "$ESC[38;2;$($CFG_Color_White)m"
$CGOLD  = "$ESC[38;2;$($CFG_Color_Gold)m"
$CBORD  = "$ESC[38;2;$($CFG_Color_Border)m"

$ScriptDir  = Split-Path -Parent $MyInvocation.MyCommand.Path
$ConfigFile = Join-Path $ScriptDir $CFG_JsonFile
$DriverFile = Join-Path $ScriptDir $CFG_DriverRelPath
...
function Show-Header {
    ...
    Write-Host ""
    Write-Color "+--------------------------------------------------------------+" $CBORD
    Write-Color "|    $($CFG_Title.PadRight(58))|" $HEADER
    Write-Color "|    Version $($CFG_Version.PadRight(51))|" $CDIM
    Write-Color "+--------------------------------------------------------------+" $CBORD
    Write-Host ""
}
...
# ================================================================
#  IP VALIDATION   XX.XX.XX.XX  each octet 0-255
# ================================================================
function Test-IP {
    param([string]$IP)
...
function Load-Config {
    ...
    try {
        ...
        Write-Color "  [WARN]  Cannot read $($CFG_JsonFile): $_" $CWARN
        Read-Key
    }
}
...
function Show-Table {
    ...
    for ($i = 0; $i -lt $script:Printers.Count; $i++) {
        $p   = $script:Printers[$i]
        $num = $i + 1
        $def = if ($p.Default) { "  $CGOLD[DEFAULT]$CR" } else { "" }
...
# ================================================================
#  DRIVER  — shared detection
# ================================================================
function Get-DriverName {
    return (Get-PrinterDriver -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -like $CFG_DriverPattern } |
        Select-Object -ExpandProperty Name -First 1)
}

# ================================================================
#  DRIVER  — EXE installation
# ================================================================
function Install-DriverExe {
    Write-Color "  [....]  Running installer. This may take several minutes..." $CWHITE
    Write-Host ""
    Start-Process -FilePath $DriverFile -ArgumentList $CFG_DriverSilentArgs -Wait
    Start-Sleep -Seconds 5
}

# ================================================================
#  DRIVER  — INF installation
# ================================================================
function Install-DriverInf {
    Write-Color "  [....]  Adding INF driver to the driver store..." $CWHITE
    Write-Host ""

    $pnp = Start-Process -FilePath "pnputil.exe" `
        -ArgumentList "/add-driver `"$DriverFile`" /install" `
        -Wait -PassThru -NoNewWindow
    Start-Sleep -Seconds 3

    if ($pnp.ExitCode -ne 0 -and $pnp.ExitCode -ne 259) {
        Write-Color "  [WARN]  pnputil exited with code $($pnp.ExitCode). Trying Add-PrinterDriver..." $CWARN
        Write-Host ""
    }

    # Attempt Add-PrinterDriver using pattern-matched name from driver store
    $infName = (Get-PrinterDriver -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -like $CFG_DriverPattern } |
        Select-Object -ExpandProperty Name -First 1)

    if (-not $infName) {
        # Fallback: try to get the driver name from the INF file itself
        $infContent = Get-Content $DriverFile -ErrorAction SilentlyContinue
        $modelLine  = $infContent | Where-Object { $_ -match '^\[Strings\]' } |
            Select-Object -First 1
        Write-Color "  [INFO]  Driver registered via pnputil. Waiting for system refresh..." $CDIM
        Start-Sleep -Seconds 5
    }
}
...
function Install-Driver {
    ...
    if (-not (Test-Path $DriverFile)) {
        Write-Color "  [ERR]   Driver file not found:" $CERR
        Write-Color "          $DriverFile" $CDIM
        Write-Color "          Check CFG_DriverRelPath in the configuration block." $CDIM
        Write-Host ""
        Show-Line
        Read-Key
        return $null
    ...
    $typeLabel = $CFG_DriverType.ToUpper()
    Write-Color "  [INFO]  Driver type : $typeLabel" $CDIM
    Write-Color "  [INFO]  Driver file : $DriverFile" $CDIM
    Write-Host ""

    if ($typeLabel -eq "INF") {
        Install-DriverInf
    } elseif ($typeLabel -eq "EXE") {
        Install-DriverExe
    } else {
        Write-Color "  [ERR]   Unknown CFG_DriverType '$CFG_DriverType'. Use EXE or INF." $CERR
        Write-Host ""
        Show-Line
        Read-Key
        return $null
    ...
    $drv = Get-DriverName
    if (-not $drv) {
        Write-Color "  [FAIL]  Driver not found after installation." $CERR
        Write-Color "          Verify CFG_DriverPattern matches the actual driver name." $CDIM
        Write-Color "          Run: Get-PrinterDriver | Select-Object Name" $CDIM
        Write-Host ""
        Show-Line
        Read-Key
        return $null
...
function Menu-Install {
    ...
    while ($true) {
        ...
        if ($raw -eq "") {
            Write-Color "  [WARN]  Empty input." $CWARN; Write-Host ""; continue
        }
        ...
        if ($raw -eq "A" -or $raw -eq "a") {
            ...
            if ($raw -notmatch '^[\d,]+$') {
                Write-Color "  [ERR]   Use digits and commas only." $CERR; Write-Host ""; continue
            }
            $bad = $false
        ...
        if ($indices.Count -eq 0) {
            Write-Color "  [WARN]  Nothing selected." $CWARN; Write-Host ""; continue
        }
...
function Menu-Add {
    ...
    if ($name -eq "") {
        Write-Color "  [ERR]   Name cannot be empty." $CERR; Read-Key; return
    }
    ...
    while ($true) {
        ...
        if (Test-IP $ip) { break }
        Write-Color "  [ERR]   Invalid IP. Required format: XX.XX.XX.XX  (0-255 per octet)" $CERR
    }

    $def = Read-Input "Set as default printer? [Y/N]:"
    $isDefault = ($def -eq "Y" -or $def -eq "y")

    if ($isDefault) { foreach ($p in $script:Printers) { $p.Default = $false } }

    $script:Printers.Add([PSCustomObject]@{ Name = $name; IP = $ip; Default = $isDefault })
    Save-Config

    Write-Host ""
    Write-Color "  [ OK ]  Printer added and saved to $CFG_JsonFile" $COK
    Read-Key
}
...
function Menu-EditIP {
    ...
    if ($inp -notmatch '^\d+$') { Write-Color "  [ERR]   Invalid number." $CERR; Read-Key; return }
    $n = [int]$inp
    if ($n -lt 1 -or $n -gt $script:Printers.Count) {
...
function Menu-SetDefault {
    ...
    $inp = Read-Input "Set which printer as default?  (Q = back):"
    if ($inp -eq "Q" -or $inp -eq "q") { return }
    if ($inp -notmatch '^\d+$') { Write-Color "  [ERR]   Invalid number." $CERR; Read-Key; return }
    $n = [int]$inp
    if ($n -lt 1 -or $n -gt $script:Printers.Count) {
...
function Menu-Manage {
    while ($true) {
        ...
        switch ($ch) {
            ...
            "5" { Menu-Remove     }
            "0" { return }
            default { Write-Color "  [WARN]  Choose 0-5." $CWARN; Start-Sleep -Seconds 1 }
        }
    }
}
...
while ($true) {
    ...
    switch ($ch) {
        ...
        "2" { Menu-Manage  }
        "0" { Clear-Host; exit 0 }
        default { Write-Color "  [WARN]  Choose 0, 1 or 2." $CWARN; Start-Sleep -Seconds 1 }
    }
}
